...
- **`main`-Schutz auf GitHub Free (privates Repo):** Serverseitige Branch-Protection/Rulesets sind hier nicht verfügbar. Ersatz: ein lokaler `.git/hooks/pre-push` blockt Direkt-Push auf `main` (nur bewusst mit `--no-verify` umgehbar; liegt lokal im Klon, ist nicht eingecheckt). Das Verhindern eines **Rot-Merges** liegt damit beim Merger — nicht am Server. Zwei erlaubte Wege, kein dritter:
  ...
  Beide Wege prüfen dieselben drei Jobs. Der maschinelle ist zulässig, weil der manuelle Blick hier Ersatz für fehlende Branch-Protection war, nicht Selbstzweck — und weil ein Skript keinen Job übersieht. Was er **nicht** ersetzt: das `hueter`-Review, das vor jedem Merge steht.
