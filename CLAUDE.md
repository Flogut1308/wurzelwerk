...
- `feld` steht nur an Existenz-Aussagen, passend zum Subjekttyp.
...
- **`main`-Schutz (Stand 14.09.2026, korrigiert):** Das Repo ist **öffentlich**, nicht privat — damit stehen serverseitige Rulesets auch auf GitHub Free zur Verfügung, und es gibt eines: Ruleset „Standard", aktiv auf `~ALL`, mit `pull_request`, `required_signatures`, `creation`, `update`, `deletion`, `non_fast_forward`. Die frühere Fassung dieses Absatzes behauptete das Gegenteil; sie stammte aus der Zeit, als das Repo privat war.
  **Es bindet derzeit nichts:** Bypass-Actor ist die Repository-Rolle *Admin* im Modus `always`. Jeder Push des Eigentümers meldet darum „Bypassed rule violations" und geht durch. `required_signatures` wird von **jedem** Commit verletzt — bisher ist keiner signiert (`git log --format='%G?'` liefert durchgehend `N`).
  Zusätzlich blockt ein lokaler `.git/hooks/pre-push` den Direkt-Push auf `main` (mit `--no-verify` umgehbar; liegt lokal im Klon, ist nicht eingecheckt). Das Verhindern eines **Rot-Merges** liegt damit beim Merger — nicht am Server. Zwei erlaubte Wege, kein dritter:
  - **Von Hand:** vor „Merge pull request“ prüfen, dass `pruefen (macos-latest)`, `pruefen (windows-latest)` und `langsame Gates (test:e2e, test:budget)` grün sind.
  - **Maschinell (seit 14.09.2026, Nachzug-Kette):** `gh pr checks <nr> --watch` und mergen nur, wenn **genau diese drei** Namen `success` melden. Fehlt einer der drei in der Ausgabe, gilt der PR als **nicht** geprüft — ein fehlender Job ist kein grüner Job. Das ist die Fassung, die `/nachzug` benutzt; sie ersetzt den Blick, nicht die Bedingung.
