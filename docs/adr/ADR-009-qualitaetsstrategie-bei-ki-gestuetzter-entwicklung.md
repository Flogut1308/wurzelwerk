...
**Zwei Generator-Profile** (hueter-Review PR #119): `bestand` — Standard für `undo-bitgleich` und alle übrigen Aufrufer — behält exakt die Gewichte von main (d0a095b) und gibt den neuen Beleg-Aktionen nur Gewicht 1 (abweichend von main ist nur die Transkript-Arbitrary von `zitatAnlegen`: optional, mit Umlaut/Emoji statt `fc.string()`); so bleiben Deckung der Bestandsbefehle und Laufzeit auf main-Niveau. `beleg` — nur `textanker-gueltig` — gewichtet die Beleg-Aktionen hoch und nutzt den Vorlauf in allen Beleg-Aktionen.
...
**Nachtrag (25.09.2026, Vorarbeiten AP-1.30 Teil 3, Eigentümer-Entscheidung; konkretisiert Punkt 8 „KI nie ohne Test einen Bugfix machen lassen“ und CLAUDE.md §5):** Rot-Beleg bei einer **reinen Leistungsänderung** (das Ergebnis bleibt bitgleich, nur die Laufzeit ändert sich). Als **Rot** gilt der Budget-Median des betroffenen Budgettests **vorher > Ziel**, als Ziel das im Auftrag genannte Leistungsziel, sonst die Budgetgrenze (bei #139: Ziel 15 ms, Grenze 20 ms), als **Grün** der Median **nachher ≤ Ziel**; beide Messprotokolle (lokal maßgeblich, CI-Wert aus `test:budget` dazu) stehen im PR-Rumpf. **Pflicht** ist zusätzlich ein **Bitgleichheits-Test gegen den alten Stand** (wortgleiche Referenz der alten Umsetzung oder kanonischer Abzug vorher/nachher), der schon **vor** der Änderung grün ist und das Verhalten festhält. Eine Budgetgrenze wird dafür weder gesenkt (sie ist kein roter Test) noch angehoben (sinngemäß ADR-025: Prüfungen werden ergänzt, nie abgeschwächt). Ändert der PR zugleich ein Ergebnis, ist er keine reine Leistungsänderung — dann gilt die eiserne Regel ohne diese Ausnahme. Erstanwendung und Muster: PR #139 (Personenliste in zwei Phasen, `test/einheit/person-liste-zwei-phasen.test.ts`).
