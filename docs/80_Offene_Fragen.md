...
| ID | Befund / Frage | Entscheidung | Konsequenz | Offen |
|---|---|---|---|---|
| **U-1.4a-imp302-beispiel2** | `beispiel-2-widersprueche.json` trägt für `tmp:erna` **kein** Geburtsdatum (weder `ereignisse[]` noch Aussage) — die absichtlich falsche Elternkante `elternschaften[1]` (`tmp:august -> tmp:erna`) kann darum mit einer korrekten IMP-302-Implementierung (braucht BEIDE Geburtsdaten, `src/core/plausibilitaet/regeln.ts`) nicht ausgelöst werden. Das illustrative Beispiel in `56_Import_Vertrag.md` §6.2 nennt für „tmp:erna" explizit „geb. 1896" — das ist eine ANDERE, hypothetische Datei (`interview-tante-erna.json`), nicht die tatsächliche Fixture. | **Nicht erfunden/geraten**, um den Fund zu erzwingen (keine Wall-Clock-Heuristik, kein Rückgriff auf `gespraechsdatum` als verdeckter Alters-Anker — beides wäre eine Annahme, die der Vertrag nicht deckt). `trockenlauf-bericht.test.ts` prüft den ehrlichen Befund (keine Stufe-3-Hinweise für `beispiel-2`); `plausibilitaet.test.ts` zeigt IMP-302 an einem Fall mit beiden Geburtsdaten. | Der Trockenlauf-Bericht für `beispiel-2` zeigt aktuell **keine** IMP-302-Meldung. | Fixture ggf. um Ernas Geburtsdatum ergänzen (ändert die deklarierte `zusammenfassung`, dann auch `import-schema-zod-gleich`/Fixture-Korpus-Tests neu ziehen) — nicht in diesem PR, da Fixture-Änderung über AP-1.4a hinausgeht. |
