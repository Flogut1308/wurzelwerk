...
**Review-Ergebnis (PR #48 — `hueter` + mechanische Verifikation):** U-DF4 (B1) und die Deckung der Systemvorgabe (D2: `@media(prefers-color-scheme: dark)` deckungsgleich mit `[data-theme="dunkel"]`) sind im PR behoben — je eigener Commit, Rot-Beleg vorab (eiserne Regel §5). Die Zählung im PR-Rumpf ist korrigiert: **139 §1-Rollen + 18 additiv = 157 `:root`-Deklarationen** (die frühere „140" zählte `--wz-schrift-koerper` doppelt — es ist zugleich §1.3-Typo-Rolle und §1.6-Dichtetoken).
