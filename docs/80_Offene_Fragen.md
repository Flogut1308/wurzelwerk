...
| ID | Befund / Frage | Entscheidung | Konsequenz | Offen |
|---|---|---|---|---|
| **U-1.6-suche-ohne-filter-sortierung-seite** | `abfrage:suche` (Stufe 1, `SucheEin`) kennt nur `text`/`grenze` — keine Filter, keine Sortierung, keine Seite. `ListenAnsicht` zeigte Filterleiste und Spaltenkopf-Sortierung anfangs auch während einer aktiven Suche klickbar, aber wirkungslos an (§14 Fall 2: kein Kontrollelement verschwindet mitten in der Bedienung). | **Nachgezogen (Nutzerentscheidung, 18.09.2026):** statt klickbar-aber-wirkungslos werden die vier Filter-Kontrollen (`Filterleiste`, Platzhalter/Privat/Widerspruch-`Umschalter` + Konfidenz-`Auswahlfeld`) und die sortierbaren Spaltenköpfe (`Datentabelle`) während einer aktiven Suche sichtbar deaktiviert (`disabled`/`aria-disabled`, gedämpfter Token-Stil `--wz-text-gesperrt`) — beide Bausteine bekamen dafür einen `gesperrt`/`sortierungGesperrt`-Zustand (`Umschalter`/`Auswahlfeld` hatten `gesperrt` schon). Filter-/Sortierzustand bleiben dabei weiter im React-Zustand erhalten und wirken sofort wieder, sobald das Suchfeld geleert wird (unverändert). Die Blätterleiste bleibt während einer aktiven Suche ausgeblendet, statt eine nicht existierende Seitenzahl vorzutäuschen. | Kein klickbar-aber-wirkungsloses Bedienelement mehr, aber auch keine UI, die während der Suche komplett verschwindet und die Bedienung unterbricht. | Keine offene Abweichung mehr. Eine künftige Erweiterung von `abfrage:suche` um Filter/Sortierung/Seite (falls Bedarf entsteht) macht die Deaktivierung überflüssig — kein Scope-Punkt dieser Stufe. |
...
| ID | Befund / Frage | Entscheidung | Konsequenz | Offen |
|---|---|---|---|---|
| **U-1.7-beziehung-platzhalter** | S-07 verlangt „Beziehungen mit Kantentyp … Platzhalter erkennbar" — aber der (gesperrte) PR-A-Vertrag `PersonDetailBeziehung` trägt für die VERWANDTE Person kein `ist_platzhalter`-Feld, nur `person_id`/`anzeigename`/`richtung`/`kantentyp`. §14 Fall 1 (geplante Funktion ohne vollen Datengrundriss) — der Vertrag war für PR-B nicht mehr änderbar (Auftrag: „NICHT ändern, nur konsumieren"). | `abl_person_ai` (`docs/schema/0003_abgeleitet.sql`) berechnet `anzeigename` aus dem bevorzugten Namen — ein Platzhalter ohne erfassten Namen (der häufige Fall, z. B. `tmp:vater-august` in `beispiel-2-widersprueche.json`) hat darum `anzeigename === ''`. `BeziehungenAbschnitt` (`profil-ansicht.tsx`) behandelt einen leeren `anzeigename` als Platzhalter (Text `platzhalter_bezeichnung`, gedämpfte Farbe) — dieselbe A-17-Regel wie in der Listenansicht, nur über den Namen statt über ein eigenes Flag erkannt. | Ein Platzhalter MIT bereits erfasstem Namen (Grund entfällt, sobald ein Name gepflegt wird) würde hier fälschlich als Nicht-Platzhalter erscheinen — seltener, aber realer Grenzfall. **Zweite, praktisch relevantere Fehlrichtung (hueter-Review PR #66, Auflage 1 — Doku-Ehrlichkeit):** Eine ECHTE Person OHNE erfassten (bevorzugten) Namen hat nach derselben Trigger-Formel (`abl_person_ai`, `TRIM(COALESCE(bn.vornamen,'') \|\| ' ' \|\| COALESCE(bn.nachname,''))`, `docs/schema/0003_abgeleitet.sql:57`) ebenfalls `anzeigename === ''` — und wird darum in `BeziehungenAbschnitt` (`src/renderer/ansichten/profil/profil-ansicht.tsx:339-340`) FÄLSCHLICH als „Platzhalter" beschriftet, obwohl dieselbe Person in der Listenansicht über ihr echtes `ist_platzhalter`-Flag (`PersonListeZeile.ist_platzhalter`) korrekt NICHT als Platzhalter geführt wird — zwei Ansichten zeigen für dieselbe Person unterschiedliche Platzhalter-Aussagen. Diese zweite Richtung ist der praktisch häufigere Fall (jede noch namenlose reale Person, nicht nur ausdrücklich markierte Platzhalter) und darum relevanter als die oben zuerst genannte. | Beide Fehlrichtungen haben dieselbe saubere Lösung: ein echtes `ist_platzhalter`-Feld in `PersonDetailBeziehung`, analog `PersonListeZeile` — das ist der bereits in dieser Zeile genannte PR-A-Folgepunkt (PR-A ist für PR-B gesperrt, bräuchte einen neuen, separat begründeten PR). Dieser Nachtrag ist ausschließlich Doku-Ehrlichkeit (hueter-Auflage 1, PR #66) — bewusst KEIN Code-Fix hier: ein Anzeige-Heuristik-Fix ohne vorherigen roten Test wäre ein Verstoß gegen die eiserne Regel (§5). |
| **U-1.7-belegliste-zweistufig** | S-08 beschreibt das Belegdetail „dreistufig": Quelle (Titel/Typ/Archiv/Signatur) → Zitat (Seite/Eintragsnummer/Zugriffsdatum/Digitalisat) → Transkript, plus `unmittelbarkeit` bei mündlichen Quellen. Der PR-A-Vertrag (`PersonDetailBeleg`) liefert nur `{ quelle: string, zitat: string \| null }` — bereits flach zusammengesetzt, keine separaten Archiv-/Signatur-/Unmittelbarkeits-Felder. §14 Fall 1. | `BelegListe`/`Widerspruchsblock` zeigen die vorhandenen zwei Stufen (Quelle-Bezeichnung, Zitat in `--wz-familie-original`) — keine erfundenen dritten/vierten Stufen, keine vorgetäuschte `unmittelbarkeit`. | Belegdetail ist knapper als der S-08-Entwurf, zeigt aber nichts Falsches. | Sobald `abfrage:person.detail` (oder ein Nachfolgevertrag) Quelle/Zitat granularer trägt, kann `BelegListe` ohne Bruch der Komponentengrenze ausgebaut werden — kein Scope-Punkt dieses PRs. |
